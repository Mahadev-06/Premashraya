      ...
      <section id="donate-form" className="bg-surface-container-low py-14 md:py-24">
        ...
        >
          ...
          <motion.div
            ...
            className="max-w-3xl mx-auto w-full bg-surface-container-lowest rounded-2xl md:rounded-[1.75rem] border border-outline-variant/20 shadow-sanctuary overflow-hidden"
          >
            {/* Accent top bar */}
              ...
              {showThankYou ? (
                ...
                <>
                  ...
                  <div className="flex border-b border-outline-variant/20 mb-6 gap-1">
                    {([
                      { id: "upi",  icon: <QrCode  className="w-4.5 h-4.5" />, label: language === "en" ? "UPI / QR Code"  : "UPI / QR କୋଡ" },
                      { id: "bank", icon: <Landmark className="w-4.5 h-4.5" />, label: language === "en" ? "Bank Transfer"  : "ବ୍ୟାଙ୍କ ହସ୍ତାନ୍ତର" },
                    ] as { id: "upi" | "bank"; icon: React.ReactNode; label: string }[]).map((tab) => (
                      <button
                        key={tab.id}
                        onClick={() => setActiveMethod(tab.id)}
                        className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 text-sm font-semibold font-sans border-b-2 transition-all duration-200 -mb-px rounded-t-lg ${
                          activeMethod === tab.id
                  ...
                  {activeMethod === "upi" && (
                    ...
                    >
                      ...
                      <div className="flex flex-col sm:flex-row gap-5 sm:gap-6 items-center sm:items-start">
                        {/* QR code */}
                        <div className="w-44 h-44 sm:w-48 sm:h-48 flex-shrink-0 bg-white rounded-2xl shadow-md border border-outline-variant/10 p-2.5 flex items-center justify-center">
                          <Image
                        ...
                        <div className="flex-1 w-full text-center sm:text-left">
                          ...
                          {copiedField === "upi" && (
                            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="font-sans text-xs text-primary mt-2 text-left">
                              {language === "en" ? "Copied to clipboard!" : "କପି ହୋଇଗଲା!"}
                            </motion.p>
                          )}
